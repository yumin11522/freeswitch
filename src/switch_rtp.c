...
static switch_status_t read_bundle_rtp_packet(switch_rtp_t *rtp_session, switch_size_t *bytes,
											  switch_frame_flag_t *flags)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	switch_rtp_t *rtp_bundle_session = switch_core_media_get_rtp_session(rtp_session->session, SWITCH_MEDIA_TYPE_AUDIO);

	if (!rtp_session) { return SWITCH_STATUS_GENERR; }
	...
	if (*bytes && rtp_bundle_session->flags[SWITCH_RTP_FLAG_DEBUG_RTP_READ]) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG_CLEAN(rtp_session->session), SWITCH_LOG_CONSOLE,
						  ...
						  rtp_session->session
							  ...
							  : "No-Name",
						  (long)*bytes, my_host, switch_sockaddr_get_port(rtp_session->local_addr), old_host,
						  rtp_session->remote_port, tx_host, switch_sockaddr_get_port(rtp_session->rtp_from_addr),
						  rtp_session->last_rtp_hdr.pt, ntohl(rtp_session->last_rtp_hdr.ts),
						  ntohs(rtp_session->last_rtp_hdr.seq), rtp_session->last_rtp_hdr.m);
	}
	...
	if (!rtp_session->flags[SWITCH_RTP_FLAG_PROXY_MEDIA] && !rtp_session->flags[SWITCH_RTP_FLAG_UDPTL]) {
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_SECURE_RECV] && rtp_session->has_rtp) {
			// if (rtp_session->flags[SWITCH_RTP_FLAG_SECURE_RECV] && (!rtp_session->ice.ice_user ||
			// rtp_session->has_rtp)) {
			int sbytes = (int)*bytes;
			srtp_err_status_t stat = 0;

			if (rtp_session->flags[SWITCH_RTP_FLAG_SECURE_RECV_RESET] ||
				!rtp_session->recv_ctx[rtp_session->srtp_idx_rtp]) {
				...
				if ((stat = srtp_create(&rtp_session->recv_ctx[rtp_session->srtp_idx_rtp],
										&rtp_session->recv_policy[rtp_session->srtp_idx_rtp])) ||
					!rtp_session->recv_ctx[rtp_session->srtp_idx_rtp]) {
			...
			if (!(*flags & SFF_PLC) && rtp_session->recv_ctx[rtp_session->srtp_idx_rtp]) {
				if (!rtp_session->flags[SWITCH_RTP_FLAG_SECURE_RECV_MKI]) {
					stat = srtp_unprotect(rtp_session->recv_ctx[rtp_session->srtp_idx_rtp],
										  &rtp_session->recv_msg.header, &sbytes);
				} else {
					stat = srtp_unprotect_mki(rtp_session->recv_ctx[rtp_session->srtp_idx_rtp],
											  &rtp_session->recv_msg.header, &sbytes, 1);
				}

				// if (rtp_session->flags[SWITCH_RTP_FLAG_NACK] && stat == srtp_err_status_replay_fail) {
				// 	/* false alarm nack */
				// 	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG1, "REPLAY ERR,
			...
			if (stat && rtp_session->recv_msg.header.pt != rtp_session->recv_te &&
	...
	if (!jb_valid(rtp_bundle_session)) { dtls_set_state(rtp_bundle_session->dtls, DS_READY); }
	...
	if (*bytes) {
		...
		if (rtp_session->has_rtp) {
			...
			if (!rtp_session->flags[SWITCH_RTP_FLAG_PROXY_MEDIA] && !rtp_session->flags[SWITCH_RTP_FLAG_UDPTL] &&
				...
				if (switch_channel_test_flag(channel, CF_AUDIO_VIDEO_BUNDLE) &&
						   ...
						   payload_map_t **pmapP, switch_frame_flag_t *flags, switch_io_flag_t io_flags)
{
	...
	rtcp:
		...
		if (!switch_channel_test_flag(channel, CF_AUDIO_VIDEO_BUNDLE) ||
			(switch_channel_test_flag(channel, CF_AUDIO_VIDEO_BUNDLE) && 
				rtp_session->flags[SWITCH_RTP_FLAG_VIDEO])) {
