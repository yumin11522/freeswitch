...
Type to_type(const std::string &type) {
    if (type == "prflx") return PeerReflexive;
    else if (type == "relay") return Relayed;
    else if (type == "srflx") return ServerReflexive;
    else return Host;
}
...
//        cout << m.at("mid") << endl;
        ...
        if (candidate_itr != m.end()) {
            for (auto cand: *candidate_itr) {
                ...
                candidate.foundation = to_string(cand.at("foundation"));
                candidate.ip = to_string(cand.at("ip"));
                candidate.port = cand.at("port");
                candidate.priority = cand.at("priority");
                candidate.protocol = to_string(cand.at("transport"));
        ...
        Content *content = new Content(name, p);
...
std::string to_sdp_str(const Session::Jingle *jingle) {
    ...
    std::stringstream ss;
    ss << "v=0\r\n";
    ss << "o=- " << session_id << " 2 IN IP4 0.0.0.0\r\n";
    ss << "s=-\r\n";
    ss << "t=0 0\r\n";
    ...
    for (auto plugin: plugins) {
        ...
        if (plugin && plugin->pluginType() == PluginContent && (content = static_cast<const Content *>(plugin))) {
            ...
            // a=fingerprint:sha-256 33:5B:D0:EA:9B:F6:92:08:8C:12:BE:D1:83:C2:A4:09:75:46:C1:E3:40:16:1F:BF:D1:98:D7:56:F1:93:8B:2F\r\n
...
std::string candidateFromJingle(Candidate &can) {
    ...
    switch (can.type) {
        ...
        case Type::ServerReflexive: {
            ss << "raddr" << " " << can.rel_addr << " rport " << can.rel_port << " ";
        }
    }
