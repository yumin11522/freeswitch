...
{
	return switch_http_request(SWITCH_HTTP_CM_POST, url, (void *)data, strlen(data), NULL, pool, 0, 0);
}

SWITCH_DECLARE(http_data_t *) switch_http_get(const char *url, switch_memory_pool_t *pool)
{
	return switch_http_request(SWITCH_HTTP_CM_GET, url, NULL, 0, NULL, pool, 0, 0);
}
					...
					switch_memory_pool_t *pool, int curl_connect_timeout, int curl_timeout)
{
	...
	if (headers) { switch_curl_easy_setopt(curl_handle, CURLOPT_HTTPHEADER, headers); }
	...
	switch_curl_easy_setopt(curl_handle, CURLOPT_USERAGENT, "gcas-curl/1.0");
...
SWITCH_DECLARE(void) switch_setup_iptables(switch_memory_pool_t *pool)
{
	...
	if (!node_ip || !pod_ip || !rtp_start || !rtp_end ||
		!switch_true(switch_core_get_variable("switch_http_setup_iptables"))) {
		return;
	}
	...
	response = switch_http_post(url, (const char *)data, pool);
